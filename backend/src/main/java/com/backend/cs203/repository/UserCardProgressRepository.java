...
public interface UserCardProgressRepository extends JpaRepository<UserCardProgress, UserCardProgressId> {
    ...
    /**
     * Insert a card progress row, silently ignoring duplicates. Uses INSERT
     * IGNORE so re-completing a card is a no-op. clearAutomatically evicts
     * stale L1 cache after the native INSERT so subsequent queries don't
     * trigger a conflicting flush.
     */
