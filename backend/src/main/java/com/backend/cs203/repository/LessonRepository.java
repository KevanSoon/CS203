...
public interface LessonRepository extends JpaRepository<Lesson, Long> {
    ...
    @Query(value = "SELECT title, description, status, created_by, created_at FROM lesson WHERE status IN ('pending','rejected','approved') AND deleted_at IS NULL ORDER BY created_at DESC", nativeQuery = true)
