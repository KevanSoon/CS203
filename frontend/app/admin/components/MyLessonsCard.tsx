...
function ReviewsPanel({ lessonId }: { lessonId: string }) {
  ...
  const countFor = (s: StarFilter) =>
    s === 0 ? baseForCount.length : baseForCount.filter((r) => r.rating === s).length;
  ...
  function handleStarFilter(s: StarFilter) { setStarFilter(s); setShowAll(false); }
  function handleFeedbackToggle() { setFeedbackOnly((v) => !v); setShowAll(false); }
                ...
                <div
                  ...
                  <div className={`flex ${isExpanded ? "flex-col sm:flex-row" : ""}`}>
					...
					className={`p-4 transition-colors ${
						record.status === "approved" ? "hover:bg-border/50 cursor-pointer" : "cursor-default"
					} ${
						isExpanded
						? "sm:w-72 sm:shrink-0 sm:border-r sm:border-border"
						: "w-full"
					}`}
