...
function StarDisplay({ value, size = 11 }: { value: number; size?: number }) {
	return (
		<div className="flex gap-0.5">
			{[1, 2, 3, 4, 5].map((i) => (
				<Star key={i} size={size} className={i <= value ? "fill-amber-400 text-amber-400" : "fill-muted text-muted-foreground/30"} />
			))}
		</div>
	);
}
...
function AvatarCircle({ username, avatarUrl }: { username: string; avatarUrl: string | null }) {
	const colours = ["bg-violet-100 text-violet-800", "bg-teal-100 text-teal-800", "bg-orange-100 text-orange-800", "bg-blue-100 text-blue-800", "bg-pink-100 text-pink-800"];
	...
	return <div className={`w-6 h-6 rounded-full flex items-center justify-center text-[10px] font-medium shrink-0 ${colours[idx]}`}>{getInitials(username)}</div>;
}
...
function ReviewsPanel({ lessonId }: { lessonId: string }) {
	...
	const filtered = useMemo(() => {
		let base = starFilter === 0 ? reviews : reviews.filter((r) => r.rating === starFilter);
		if (feedbackOnly) base = base.filter((r) => r.feedback?.trim());
	...
	const countFor = (s: StarFilter) => (s === 0 ? baseForCount.length : baseForCount.filter((r) => r.rating === s).length);
	...
	return (
		<div className="bg-muted/30 p-4 h-full">
			{fetchState === "loading" && (
			...
			{fetchState === "error" && <p className="text-xs text-destructive text-center py-4">Failed to load reviews.</p>}
			{fetchState === "done" && reviews.length === 0 && <p className="text-xs text-muted-foreground text-center py-4">No reviews yet.</p>}

			{fetchState === "done" && reviews.length > 0 && (
				<div className="flex flex-col sm:flex-row gap-4 items-start">
					{/* ── Left: score + bars + filter pills ── */}
					<div className="w-full sm:w-44 shrink-0">
						<div className="flex items-start gap-2">
							<div className="text-center min-w-[44px]">
								<p className="text-2xl font-semibold text-foreground leading-none">{avg.toFixed(1)}</p>
									...
									return (
										<div key={s} className="flex items-center gap-1.5">
											...
											<div className="flex-1 h-1 bg-border rounded-full overflow-hidden">
												<div className="h-full bg-amber-400 rounded-full transition-all" style={{ width: `${Math.round((total / maxTotal) * 100)}%` }} />
											</div>
						...
						{/* Filter pills — compact wrapping grid */}
						<div className="mt-3">
                      ...
                      ${
							starFilter === s && !feedbackOnly
								...
								: "bg-background text-muted-foreground border-border hover:border-primary/30 hover:text-foreground"
										...
										{s === 0 ? "All" : `${s} ★`} <span className="opacity-60">{countFor(s)}</span>
									</button>
								))}
							</div>
							...
							<button
								...
								className={`text-[11px] px-2.5 py-0.5 rounded-full border transition-colors
                  ${feedbackOnly ? "bg-primary/10 text-primary border-primary/30 font-medium" : "bg-background text-muted-foreground border-border hover:border-primary/30 hover:text-foreground"}`}>
								With Feedback <span className="opacity-60">{feedbackCount}</span>
							</button>
						</div>
					</div>

					{/* ── Right: review cards ── */}
					<div className="flex-1 min-w-0 w-full max-h-[400px] overflow-y-auto pr-1">
						{filtered.length === 0 ? (
				...
				<div className="overflow-hidden">
					<div className="space-y-4">
						{displayData.map((record, i) => {
							...
							return (
								<div key={i} className={`rounded-lg border border-border transition-colors overflow-hidden` + (record.deletedAt ? " opacity-50 pointer-events-none grayscale" : "")}>
									{/* Card body — flex-col on mobile, flex-row on desktop when expanded */}
									<div className={`flex ${isExpanded ? "flex-col sm:flex-row" : ""}`}>
										{/* ── Left: lesson info ── */}
										<div
											className={`p-4 transition-colors ${record.status === "approved" ? "hover:bg-border/50 cursor-pointer" : "cursor-default"} ${
												isExpanded ? "sm:w-72 sm:shrink-0 sm:border-r sm:border-border" : "w-full"
											}`}
											onClick={() => record.status === "approved" && toggleLessonExpand(String(record.id))}>
										...
										{/* ── Right: reviews panel ──
                        Mobile: stacks below (flex-col), border-t separates sections
                        Desktop: sits beside (sm:flex-row), sm:border-t-0 removes top border
                        since sm:border-r on the left panel acts as divider instead        */}
										{isExpanded && record.status === "approved" && (
											<div className="flex-1 min-w-0 border-t border-border sm:border-t-0">
												<ReviewsPanel lessonId={String(record.id)} />
											</div>
										)}
									</div>
								</div>
