...
export async function DELETE(request: Request, context: { params: Promise<{ lessonId: string }> }
    ...
    try {
        const {data} = await axios.delete(`${BACKEND_URL}/api/progress/lesson/${lessonId}`, {
            headers: {Authorization: `Bearer ${jwt}`},
        });
        ...
        console.error("Backend request failed:", err.message);
        return Response.json(
            { error: err?.response?.error || "Internal Server Error", message: err?.response?.data?.message || "Progress Removal failed. Please try again" },
            { status: err?.response?.status || 500 }
        );
    }
}